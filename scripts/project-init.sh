...
tmp_project= tmp_models= tmp_skills= tmp_agents_conf= tmp_workflows= tmp_context= tmp_agents= expected_project= expected_models= expected_skills= expected_agents_conf= expected_workflows= expected_context= expected_agents=
usage() { cat >&2 <<'EOF'
...
cleanup() { status=$?; for file in "$tmp_project" "$tmp_models" "$tmp_skills" "$tmp_agents_conf" "$tmp_workflows" "$tmp_context" "$tmp_agents" "$expected_project" "$expected_models" "$expected_skills" "$expected_agents_conf" "$expected_workflows" "$expected_context" "$expected_agents"; do [ -n "$file" ] && [ -f "$file" ] && [ ! -L "$file" ] && rm -f "$file" || :; done; return "$status"; }
trap 'cleanup' EXIT
...
expected_skills=$(mktemp "${TMPDIR:-/tmp}/ccb-skills.XXXXXX") || exit 1; expected_agents_conf=$(mktemp "${TMPDIR:-/tmp}/ccb-agents-conf.XXXXXX") || exit 1; expected_workflows=$(mktemp "${TMPDIR:-/tmp}/ccb-workflows.XXXXXX") || exit 1
printf 'CCB_PROJECT_NAME=%s\nCCB_PROJECT_PROFILE=%s\nCCB_PROJECT_VERSION=1\nCCB_TEMPLATE_VERSION=1.8.0\n' "$project_name" "$profile" >"$expected_project" || exit 1
...
project_workflows_default_content >"$expected_workflows" || exit 1
printf '# Project context\n\nProject: %s\nProfile: %s\n\n## Profile guidance\n\n%s\n\nSafety: %s\n\n## Skills\n\nPonytail: %s\nPonytail mode: %s\nSource: DietrichGebert/ponytail\n\n## Agent roles\n\n- manager — read-only\n- graph — read-only\n- graphiste — write\n- developer — write\n- reviewer — read-only\n\nAccess levels are declarative in V1.7.x and are not an operating-system sandbox.\n\n## Workflows\n\nDefault workflow: feature\n\n- feature — manager -> developer -> reviewer\n- bugfix — graph -> developer -> reviewer\n- design — manager -> graphiste -> reviewer\n- review — reviewer\n\nWorkflow definitions are declarative. Runtime execution requires an explicit workflow command.\n\n## Model routing\n\nProvider: ollama\nDefault: %s\nPlanner: %s\nCoder: %s\nReviewer: %s\n' "$project_name" "$profile" "$PROJECT_PROFILE_GUIDANCE" "$PROJECT_PROFILE_SECURITY" "$ponytail_status" "$ponytail_mode" "$default_model" "$planner" "$coder" "$reviewer" >"$expected_context" || exit 1
printf '# Agent guidance\n\nRead .ccb/context/project.md before modifying files.\nRead .ccb/models.conf before selecting a model or agent role.\nRead .ccb/skills.conf before applying project skills.\nRead .ccb/agents.conf before selecting an agent role.\nRead .ccb/workflows.conf before selecting a workflow.\nRespect the selected workflow order; do not silently skip a step. Workflows are declarative and do not automatically execute agents or authorize Git or system actions.\nRespect the active role: read-only roles provide analysis or recommendations only; write roles change only what is necessary and verify first. Access is declarative only, not an operating-system sandbox.\n\n## Ponytail\n\nMode: %s\nSource: DietrichGebert/ponytail\n\n%s\n' "$ponytail_mode" "$ponytail_rules" >"$expected_agents" || exit 1

conflicts= messages= state_project=CREATE state_models=CREATE state_skills=CREATE state_agents_conf=CREATE state_workflows=CREATE state_context=CREATE state_agents=CREATE
...
if [ "$new_target" -eq 0 ] && { [ -L "$target/.ccb" ] || { [ -e "$target/.ccb" ] && [ ! -d "$target/.ccb" ]; }; }; then conflict 'error: incompatible parent path: .ccb'; state_project=CONFLICT; state_models=CONFLICT; state_context=CONFLICT
  ...
  classify "$target/.ccb/workflows.conf" "$expected_workflows" .ccb/workflows.conf; state_workflows=$result
...
printf 'Plan:\n  %s .ccb/project.conf\n  %s .ccb/models.conf\n  %s .ccb/skills.conf\n  %s .ccb/agents.conf\n  %s .ccb/workflows.conf\n  %s .ccb/context/project.md\n  %s AGENTS.md\n' "$state_project" "$state_models" "$state_skills" "$state_agents_conf" "$state_workflows" "$state_context" "$state_agents"
...
prepare() { dest=$1 expected=$2 kind=$3 dir=$(dirname "$dest"); base=$(basename "$dest"); temp=$(mktemp "$dir/.${base}.tmp.XXXXXX") || return 1; cp "$expected" "$temp" && chmod 644 "$temp" || { rm -f "$temp" || :; return 1; }; case "$kind" in project) tmp_project=$temp;; models) tmp_models=$temp;; skills) tmp_skills=$temp;; agents_conf) tmp_agents_conf=$temp;; workflows) tmp_workflows=$temp;; context) tmp_context=$temp;; agents) tmp_agents=$temp;; esac; }
prepare_all() { [ "$state_project" != CREATE ] || prepare "$target/.ccb/project.conf" "$expected_project" project || return 1; [ "$state_models" != CREATE ] || prepare "$target/.ccb/models.conf" "$expected_models" models || return 1; [ "$state_skills" != CREATE ] || prepare "$target/.ccb/skills.conf" "$expected_skills" skills || return 1; [ "$state_agents_conf" != CREATE ] || prepare "$target/.ccb/agents.conf" "$expected_agents_conf" agents_conf || return 1; [ "$state_workflows" != CREATE ] || prepare "$target/.ccb/workflows.conf" "$expected_workflows" workflows || return 1; [ "$state_context" != CREATE ] || prepare "$target/.ccb/context/project.md" "$expected_context" context || return 1; [ "$state_agents" != CREATE ] || prepare "$target/AGENTS.md" "$expected_agents" agents || return 1; }
...
if ! finalize "$tmp_project" "$target/.ccb/project.conf" || ! finalize "$tmp_models" "$target/.ccb/models.conf" || ! finalize "$tmp_skills" "$target/.ccb/skills.conf" || ! finalize "$tmp_agents_conf" "$target/.ccb/agents.conf" || ! finalize "$tmp_workflows" "$target/.ccb/workflows.conf" || ! finalize "$tmp_context" "$target/.ccb/context/project.md" || ! finalize "$tmp_agents" "$target/AGENTS.md"; then error 'failed to finalize one or more managed files'; exit 1; fi
tmp_project= tmp_models= tmp_skills= tmp_agents_conf= tmp_workflows= tmp_context= tmp_agents=
