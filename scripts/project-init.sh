...
target= project_name= profile=generic profile_set=0 model= planner_model= coder_model= reviewer_model= yes=0 dry_run=0 new_target=0
tmp_project= tmp_models= tmp_context= tmp_agents= expected_project= expected_models= expected_context= expected_agents=
usage() { cat >&2 <<'EOF'
...
cleanup() { status=$?; for file in "$tmp_project" "$tmp_models" "$tmp_context" "$tmp_agents" "$expected_project" "$expected_models" "$expected_context" "$expected_agents"; do [ -n "$file" ] && [ -f "$file" ] && [ ! -L "$file" ] && rm -f "$file" || :; done; return "$status"; }
trap 'cleanup' EXIT
...
printf '# Project context\n\nProject: %s\nProfile: %s\n\n## Profile guidance\n\n%s\n\nSafety: %s\n\n## Model routing\n\nProvider: ollama\nDefault: %s\nPlanner: %s\nCoder: %s\nReviewer: %s\n' "$project_name" "$profile" "$PROJECT_PROFILE_GUIDANCE" "$PROJECT_PROFILE_SECURITY" "$default_model" "$planner" "$coder" "$reviewer" >"$expected_context" || exit 1
printf '# Agent guidance\n\nRead .ccb/context/project.md before modifying files.\nRead .ccb/models.conf before selecting a model or agent role.\nFollow project safety conventions.\n' >"$expected_agents" || exit 1

conflicts= messages= state_project=CREATE state_models=CREATE state_context=CREATE state_agents=CREATE
...
printf 'Plan:\n  %s .ccb/project.conf\n  %s .ccb/models.conf\n  %s .ccb/context/project.md\n  %s AGENTS.md\n' "$state_project" "$state_models" "$state_context" "$state_agents"
...
prepare() { dest=$1 expected=$2 kind=$3 dir=$(dirname "$dest"); base=$(basename "$dest"); temp=$(mktemp "$dir/.${base}.tmp.XXXXXX") || return 1; cp "$expected" "$temp" && chmod 644 "$temp" || { rm -f "$temp" || :; return 1; }; case "$kind" in project) tmp_project=$temp;; models) tmp_models=$temp;; context) tmp_context=$temp;; agents) tmp_agents=$temp;; esac; }
prepare_all() { [ "$state_project" != CREATE ] || prepare "$target/.ccb/project.conf" "$expected_project" project || return 1; [ "$state_models" != CREATE ] || prepare "$target/.ccb/models.conf" "$expected_models" models || return 1; [ "$state_context" != CREATE ] || prepare "$target/.ccb/context/project.md" "$expected_context" context || return 1; [ "$state_agents" != CREATE ] || prepare "$target/AGENTS.md" "$expected_agents" agents || return 1; }
...
if ! finalize "$tmp_project" "$target/.ccb/project.conf" || ! finalize "$tmp_models" "$target/.ccb/models.conf" || ! finalize "$tmp_context" "$target/.ccb/context/project.md" || ! finalize "$tmp_agents" "$target/AGENTS.md"; then error 'failed to finalize one or more managed files'; exit 1; fi
tmp_project= tmp_models= tmp_context= tmp_agents=
