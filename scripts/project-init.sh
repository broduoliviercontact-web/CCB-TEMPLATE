...
tmp_project= tmp_models= tmp_skills= tmp_context= tmp_agents= expected_project= expected_models= expected_skills= expected_context= expected_agents=
usage() { cat >&2 <<'EOF'
...
cleanup() { status=$?; for file in "$tmp_project" "$tmp_models" "$tmp_skills" "$tmp_context" "$tmp_agents" "$expected_project" "$expected_models" "$expected_skills" "$expected_context" "$expected_agents"; do [ -n "$file" ] && [ -f "$file" ] && [ ! -L "$file" ] && rm -f "$file" || :; done; return "$status"; }
trap 'cleanup' EXIT
...
expected_skills=$(mktemp "${TMPDIR:-/tmp}/ccb-skills.XXXXXX") || exit 1
...
conflicts= messages= state_project=CREATE state_models=CREATE state_skills=CREATE state_context=CREATE state_agents=CREATE
...
if ! finalize "$tmp_project" "$target/.ccb/project.conf" || ! finalize "$tmp_models" "$target/.ccb/models.conf" || ! finalize "$tmp_skills" "$target/.ccb/skills.conf" || ! finalize "$tmp_context" "$target/.ccb/context/project.md" || ! finalize "$tmp_agents" "$target/AGENTS.md"; then error 'failed to finalize one or more managed files'; exit 1; fi
tmp_project= tmp_models= tmp_skills= tmp_context= tmp_agents=
