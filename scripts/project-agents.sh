...
if [ "$action" = show ]; then
  ...
  project_agent_details "$requested_role"; model=$(project_agent_model_value "$PROJECT_AGENT_MODEL_ROLE") || exit 1
...
for role in manager graph graphiste developer reviewer; do project_agent_details "$role"; model=$(project_agent_model_value "$PROJECT_AGENT_MODEL_ROLE") || exit 1; printf '%-12s %-12s %-10s %s\n' "$role" "$PROJECT_AGENT_ACCESS" "$PROJECT_MODEL_PROVIDER" "$model"; done
