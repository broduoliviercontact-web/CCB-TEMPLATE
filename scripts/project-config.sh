...
models_output=$(printf 'Model configuration: %s\nDefault model: %s\nPlanner model: %s\nCoder model: %s\nManager model: %s\nGraph model: %s\nGraphiste model: %s\nDeveloper model: %s\nReviewer model: %s\nFallback model: %s' "$PROJECT_MODEL_FORMAT" "$PROJECT_MODEL_DEFAULT" "$PROJECT_MODEL_PLANNER" "$PROJECT_MODEL_CODER" "$PROJECT_MODEL_MANAGER" "$PROJECT_MODEL_GRAPH" "$PROJECT_MODEL_GRAPHISTE" "$PROJECT_MODEL_DEVELOPER" "$PROJECT_MODEL_REVIEWER" "$PROJECT_MODEL_FALLBACK")
printf 'Project: %s\nProfile: %s\nProvider: %s\n%s\n%s\n%s\n%s\n%s\n' "$PROJECT_NAME" "$PROJECT_PROFILE" "$PROJECT_MODEL_PROVIDER" "$models_output" "$skills_output" "$agents_output" "$workflows_output" "$runs_output"
