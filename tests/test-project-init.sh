...
for option in TARGET --project-name --profile --yes --dry-run; do assert_contains "$output" "$option" 'init help documents options'; done
...
for file in .ccb/project.conf .ccb/context/project.md AGENTS.md; do assert_file "$existing/$file"; done
...
for file in .ccb/project.conf .ccb/context/project.md AGENTS.md; do assert_contains "$output" "SKIP $file" 'idempotent plan'; done
