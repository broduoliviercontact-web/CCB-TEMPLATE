...
Start Ollama and run `ollama list`. The required models are `glm-5.3-flash:cloud`
(manager, graph and reviewer) and `kimi-k2.7-code:cloud` (developer). A model-retired
HTTP 410 response means that the requested model must be replaced; do not restore the
removed V1 coding fallback.
