...
Use `./scripts/ccb.sh init TARGET` to create the smallest local CCB project contract. The only supported profile is `generic`; the command does not access the network, initialize Git, install software, configure models, or call a provider.
...
The managed files are exactly `.ccb/project.conf`, `.ccb/context/project.md`, and `AGENTS.md`. Their expected contents are newline-terminated. Unknown project files are left unchanged.

Before writing, the command prints a complete plan for all managed files: `CREATE` for an absent path, `SKIP` for an identical regular file, and `CONFLICT` for different content, a symbolic link, a non-regular file, or an incompatible parent. A conflict exits with status 1 and prevents every write. A second identical invocation therefore reports three `SKIP` entries.
